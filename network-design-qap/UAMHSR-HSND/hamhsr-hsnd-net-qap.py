...
def run_case(cwd,time_limit):
    ...
    # write solution in csv files 

    if model.SolCount == 0:
        print("No feasible solution found")
    else:
        # open a file for writing
        open_path = os.path.join(cwd,'output_path_solution(net-qap).csv')
        open_assignment_1 = os.path.join(cwd,'output_assignment_solution_1(net-qap).csv')
        open_assignment_2 = os.path.join(cwd,'output_assignment_solution_2(net-qap).csv')
        open_performance = os.path.join(cwd,'output_performance(net-qap).csv')
        ...
        # report best objective value and computation time and gap, the time have the first feasible solution
...
if __name__ == "__main__":
    ...
    for case in case_list:
        cwd = case[0]
        time_limit = case[1]
        print("**********===== Running case: ", cwd, " using net-qap model ============************")
