...
def run_case(cwd,First_step_time,Second_step_time):
    ...
    # stop if get feasible solution with gap less than 50%
    model.optimize()
    ...
    open_performance = os.path.join(cwd,'output_performance(hybrid-a-frieze-yadegar)-step1.csv')
    ...
    if model.MIPGap>0.01:
        ...
        model.Params.MIPGap = 1e-4  # set MIP gap to 0.01% to speed up the first step
        model.optimize()
    ...
    if model.SolCount == 0:
        ...
        open_path = os.path.join(cwd,'output_path_solution(hybrid-a-frieze-yadegar).csv')
        open_assignment_1 = os.path.join(cwd,'output_assignment_solution_1(hybrid-a-frieze-yadegar).csv')
        open_assignment_2 = os.path.join(cwd,'output_assignment_solution_2(hybrid-a-frieze-yadegar).csv')
        open_performance = os.path.join(cwd,'output_performance(hybrid-a-frieze-yadegar).csv')
...
if __name__ == "__main__":
    ...
    for case in case_list:
        ...
        print("**********===== Running case: ", cwd, " using hybrid a-frieze-yadegar model ============************")
