...
class ParamsQAP:
    def __init__(self, cwd='', args=None):
        ...
        self.nb_of_branch = 0
        self.branch_list = []
        # self.branch_id_pair_dict = {}
        self.branch_fromtoid_to_branch_dict = {}
    ...
    def initialization(self):
        ...
        for link in self.link_list:
            if link.link_type == 2:  # 'building_orig'
                ...
                i_ind = np.mod(branch.from_node_id, node_id_unit)
                k_ind = np.mod(branch.to_node_id, node_id_unit)
                branch.i_ind, branch.k_ind = i_ind, k_ind
                branch.init_assignment_mat = np.matmul(self.trans_cost_mat[k_ind - 1:k_ind].T, self.flow_mat[i_ind - 1:i_ind])  # do not consider the building cost in l-j side
                # self.init_assignment_mat[k_ind - 1, i_ind - 1] = -M
                branch.branch_id = self.nb_of_branch
                self.branch_fromtoid_to_branch_dict[(branch.from_node_id, branch.to_node_id)] = branch
                self.nb_of_branch += 1

                branch.assignment_mat = branch.init_assignment_mat  # in the first iteration, intial assignment matrices are used
                branch.assignment_count = np.zeros([self.nb_of_dest_location, self.nb_of_dest_building])
        ...
        solutions = pool.map(GLB, input_list)
        for solution in solutions:
            branch = self.branch_list[solution['branch_seq']]
            branch.lower_value = solution['value']
            ...
            for i in range(self.nb_of_dest_location):
                xx = branch.location_ind[i]
                yy = branch.building_ind[i]
                branch.assignment_count[xx][yy] += 1

        for branch in self.branch_list:
            i_ind = np.mod(branch.from_node_id, node_id_unit)
            k_ind = np.mod(branch.to_node_id, node_id_unit)
            self.GLB_cost_mat[i_ind - 1][k_ind - 1] = branch.lower_value
...
def GLB(input_list):
    branch_seq, assignment_mat = input_list
    location_ind, building_ind = linear_sum_assignment(assignment_mat)
    value = assignment_mat[location_ind, building_ind].sum()
    # + M # As this is the symmetric case, the i,k of each branch must be chosen in its corresponding l,j
